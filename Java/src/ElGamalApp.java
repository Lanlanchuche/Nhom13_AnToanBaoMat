...
//    static final Color BUTTON_SEC_HOVER = new Color(51, 71, 102); // #334766
    ...
    private BigInteger sharedP = BigInteger.ZERO, sharedG = BigInteger.ZERO,
            sharedX = BigInteger.ZERO, sharedY = BigInteger.ZERO;
    ...
    private JPanel buildKeyTab() {
        ...
        Runnable refreshDisplay = () -> {
            if (sharedP.equals(BigInteger.ZERO)) {
        ...
        btnAuto.addActionListener(e -> {
            BigInteger[] keys = ElGamal.generateKey();
        ...
        btnManual.addActionListener(e -> {
            try {
                BigInteger p = new BigInteger(tfP.getText().trim());
                BigInteger g = new BigInteger(tfG.getText().trim());
                BigInteger x = new BigInteger(tfX.getText().trim());
                ...
                if (x.compareTo(BigInteger.valueOf(2)) < 0 || x.compareTo(p.subtract(BigInteger.ONE)) >= 0)
                    throw new Exception("x phải nằm trong khoảng [2, p-2]!");
                BigInteger computedY = ElGamal.modPow(g, x, p);
                BigInteger y = tfY.getText().trim().isEmpty() ? computedY : new BigInteger(tfY.getText().trim());
                if (!y.equals(computedY)) throw new Exception("y không khớp với g^x mod p!");
    ...
    // =====================================================================
    //  TAB 2 – MÃ HÓA (ĐÃ SỬA MÀU CHỮ TIÊU ĐỀ THÀNH TRẮNG)
    // =====================================================================
    private JPanel buildEncryptTab() {
        ...
        // ĐÃ SỬA: Thay thế new JLabel(...) bằng whiteLabel(...) giúp chữ biến thành màu trắng rõ nét
        ...
        btnEncrypt.addActionListener(e -> {
            try {
                BigInteger p = new BigInteger(encP.getText().trim());
                BigInteger g = new BigInteger(encG.getText().trim());
                BigInteger y = new BigInteger(encY.getText().trim());
                ...
                if (encK.getText().trim().isEmpty()) {
                    k = ElGamal.generateK(p);
                    encK.setText(k.toString());
                } else {
                    k = new BigInteger(encK.getText().trim());
                }

                StringBuilder sb = new StringBuilder();
                for (char c : plain.toCharArray()) {
                    BigInteger[] pair = ElGamal.encryptChar(BigInteger.valueOf(c), p, g, y, k);
    ...
    // =====================================================================
    //  TAB 3 – GIẢI MÃ (ĐÃ SỬA MÀU CHỮ TIÊU ĐỀ THÀNH TRẮNG)
    // =====================================================================
    private JPanel buildDecryptTab() {
        ...
        btnDecrypt.addActionListener(e -> {
            ...
            try {
                BigInteger p = new BigInteger(currentP);
                BigInteger x = new BigInteger(currentX);
                ...
                for (String[] pair : pairs) {
                    BigInteger c1 = new BigInteger(pair[0].trim());
                    BigInteger c2 = new BigInteger(pair[1].trim());
